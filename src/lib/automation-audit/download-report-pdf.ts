import { saveReportPdf } from "@/lib/automation-audit/generate-report-pdf";
import type { AutomationAuditReport } from "@/lib/automation-audit/types";

export function downloadReportPdf(report: AutomationAuditReport): void {
...
export function downloadReportJson(report: unknown, hostname: string): void {
  ...
  a.href = url;
  a.download = `automation-audit-${safeHost}-${date}.json`;
  a.click();
